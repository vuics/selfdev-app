...
const Prices = () => {
  ...
  useEffect(() => {
    const fetchPrices = async () => {
      ...
      const {prices} = await fetch(`${conf.api.url}/subscriptions/config`).then(r => r.json());
      setPrices(prices);
    };
    fetchPrices();
  }, [])
  ...
  const createSubscription = async (priceId) => {
    ...
    const {subscriptionId, clientSecret} = await fetch(`${conf.api.url}/subscriptions/create-subscription`, {
      method: 'POST',
      headers: {
